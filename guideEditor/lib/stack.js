...
Stack.prototype = {
     ...
     *@return {Group} founded group of null if none founded
     **/
    ...
    setPosition_deprecated:function(figure, position){
        ...
        if(figureIndex!=-1 && position>=0 && position<this.figures.length){
            var tempFigure=this.figures.splice(figureIndex,1);
    ...
    /**Test if an (x,y) is over a Container's edge (right on its edge)
     *@param {Number} x - the x coordinates
     *@param {Number} y - the y coordinates
     *@return {Boolean} - true if over a container, false otherwise
     **/
    ...
    getWorkAreaBounds:function() {
        ...
        var compareAndSet = function (bounds){
            ...
            // if once function were ran - one object seated it's bounds
            unset = false;
        };
        var i;
