 ...
 *
 *ALL VARIABLES DEFINED HERE WILL BE VISIBLE IN ALL OTHER MODULES
 *SO TAKE CARE!
 **/
 ...
 * must be increased and also update the importer.js library
 * */
...
/**
 *Scrollbar width
 *19px is the width added to a scrollable area (discovered this into Chrome)
...
var INDENTATION = 0;
/**
 *Supposedly stop any selection from happening
 */
...
var snapTo = false;
/**Keeps last coordinates while dragging*/
var lastClick = [];
/**Default line width*/
...
/**Current selected connector (-1 if none selected)*/
var selectedConnectorId = -1;
/**Current selected container (-1 if none selected)*/
...
function updateShape(shapeId, property, newValue, skipCommand, previousValue){
    ...
    //TODO: this horror must disappear
    if(!obj){
    ...
    if(propSet in obj){ //@see https://developer.mozilla.org/en/JavaScript/Reference/Operators/Special_Operators/in_Operator
         ...
         *  likelihood is it will need access to its parent figure.
...
/**Makes grid visible or invisible, dependinding of previous value
 *If the "snap to" was active and grid made invisible the "snap to" will be disabled
 **/
function showGrid(){
...
//            break;
        ...
        case KEY.V:
            if(CNTRL_PRESSED){
                 ...
                 *      - if group copied then check if it`s not permanent, because then we have lost it, and if not, then duplicate it
 ...
 *"Because of the snap to grid function we need to move more than a certain amount of pixels
 ...
 *So snapMonitor = [sumOfChangesOnX, sumOfChangesOnY]
 **/
var snapMonitor = [0,0];
...
function onMouseMove(ev){
    ...
    switch(state){
        ...
        case STATE_SELECTING_MULTIPLE:
            ...
            selectionArea.points[2].y = y; //top left
 ...
 * but OpenOffice's Draw is using something similar to that project
...
function linkMap(){
    ...
    Log.info("linkMap()->csv bounds: " + csvBounds);
    return csvBounds;
}
...
/**Minimap section*/
var minimap; //stores a reference to minimap object (see minimap.js)
...
//            break;
        ...
        case 'shrink':
            ...
            break;
        case 'duplicate':
            //TODO: Connectors are not cloned
    ...
    switch(state){
        case STATE_FIGURE_CREATE:
            ...
            draggingFigure.addEventListener('mouseup', function (ev){
                ...
                //stop canvas from getting this event
                evt.stopPropagation();
            }, false);
            break;
        case STATE_NONE:
