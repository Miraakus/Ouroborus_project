...
/**
 *Handles are created on-the-fly for a figure. They are completely managed by the HandleManager
 *Each handle is responsible for an action. The {Handle} does not need to keep a reference to the parent shape
 ...
 * Note: More handles might be added in the future : like handles to increase the number of edges for a hexagon
...
Handle.prototype = {    
    ...
    actionShape: function(lastMove, newX, newY){
        ...
        if(this.type == 'r'){
            ...
            //find the angle by which the figure was rotated (for any figure this is initially 0 so if != than 0 we have a rotation)
            ...
            switch(this.type){
                case 'n':
                    ...
                    break;
                case 's':
                    /*move the xOy coordinates at the top of the figure (superfluous as we are there already) and then scale*/
                    transY = figBounds[1];
                ...
                case 'w':
                    ...
                    break;
                case 'e':
                    /*move the xOy coordinates at the left of the figure (superfluous as we are there already) and then scale*/
                    transX = figBounds[0];
            ...
            /*By default the NW, NE, SW and SE are scaling keeping the ratio
            ...
            //move the figure to origin and "unrotate" it
    ...
    actionConnector: function(lastMove, newX, newY){
        switch(this.type){
            case 'v':
                 ...
                 */
                //Pick turning points neighbors and translate them
    ...
    getCursor:function(){
        if(HandleManager.shape instanceof Connector){
            ...
            //find north
            var closestToNorthIndex = -1; //keeps the index of closest handle to North
            var minAngleToNorth = 2 * Math.PI; //keeps the smallest (angular) distance to North
            for(var i=0; i<HandleManager.handles.length-1; i++){
                var handleCenter = new Point(HandleManager.handles[i].x, HandleManager.handles[i].y);
                var angle = Util.getAngle(figureCenter,handleCenter); //get the angle between those 2 points 0=n
                if(angle <= Math.PI){ //quadrant I or II
...
/**Use this method to set a new shape (Figure or Connector) to this manager.
 * Every time a new figure is set, old handles will disappear (got erased by new figure's handles)
 **/
