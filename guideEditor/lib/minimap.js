...
function Minimap(bigCanvas, minimapContainer){
    ...
    this.selection.onmousedown = function(aMinimapObject){
        return function(event){
            /*prevent Firefox to allow canvas drag effect. By default FF allows you
...
/**Preferred/default height*/
Minimap.prefferedHeight = 250;

Minimap.prototype = {
    constructor : Minimap,    
    /**
     *Update the minimap (canvas) with a scaled down version of the big map (canvas)
     *@author Zack
     *TODO: because of this the whole painting is very slow. We need to optimize it
     *Idea: make update not real time....but with a delay...1 s for example  so
