...
Point.prototype = {
    ...
    /**
     *We will draw a point a circle. The "visual" color and thickness of the point will
...
/**
  * Creates an instance of a Line. A Line is actually a segment and not a pure geometrical Line
  * @constructor
  * @this {Line}
...
Polyline.prototype = {
    ...
    /**Return the {Point} corresponding the t certain t value. 
     * NOTE: It is the visual percentage of a point from the start of the 
     * primitive and the result is different than the t from getPoint(...) method
...
/**
  ...
  * DottedPolygon is a polygon with all edges dotted or with a certain pattern.
  * As for now (Summer 2012) context does not support lines with a pattern
  * this had to be created
  * @constructor
  * @this {DottedPolygon}
  * @param {Array} pattern - an array of dots and spaces Ex: [10,2,2 4,7] means 10 dots, 2 spaces, 2 dots and 7 spaces.
  **/
...
QuadCurve.prototype = {
    ...
    //TODO: dynamically adjust until the length of a segment is small enough (1 unit)?
    getPoints:function(){
        var STEP = 0.01;
        var points = [];
     ...
     * TODO: This is by far not the best algorithm but only an approximation.
     * */
    getLength:function(){
        /*Algorithm: split the Bezier curve into an approximate 
    ...
    /*We could use an interpolation algorithm t=0,1 and pick 10 points to iterate on ...but for now it's fine
     **/
    getBounds:function(){
        return Util.getBounds(this.getPoints());
    },
...
CubicCurve.prototype = {
     ...
     */
    contains:function(x, y) {
        /*Algorithm: split the Bezier curve into an approximate polyline and 
...
function Arc(x, y, radius, startAngle, endAngle, direction, styleFlag){
    ...
    for (var i = 0; i < numControlPoints; i++) {
        ...
        //control radius formula where does it come from, why does it work?
        ...
        //if we aren`t starting at 0, rotate it to where it needs to be
        //move to origin (O)
    ...
    /**The style flag - see  constructor's arguments*/
 ...
 * @constructor
 * @this {DashedArc}
 * @param {Number} x - x coordinated of the center of the "invisible" circle
 * @param {Number} y - y coordinated of the center of the "invisible" circle
    ...
    toSVG: function(){
        ...
        for(var i=0; i<this.primitives.length; i++){
            ...
            if(primitive instanceof Line){
                 ...
                 *the M is unnecessary - maybe we are already coming from that spot*/
...
Figure.load = function(o){
    ...
    for(var i=0; i< o.primitives.length; i++){
        ...
        if(o.primitives[i].oType == 'Point'){
            ...
            newFigure.addPrimitive(Figure.load(o.primitives[i])); //kind a recursively
        }
        else if(o.primitives[i].oType == 'ImageFrame'){
            newFigure.addPrimitive(ImageFrame.load(o.primitives[i])); //kind a recursively
        }
    }
...
Figure.prototype = {    
    ...
    applyAnotherFigureStyle:function(anotherFigure){
        ...
        //TODO:there is some problem if applying text twice, the getText returns empty string, this means it is not properly cloned
...
function NURBS(points){
    if(points.length < 2){
        throw "NURBS: constructor() We need minimum 3 points to have a NURBS";
    }
...
NURBS.prototype = {
    /**Computes a series of Bezier(Cubic) curves to approximate a curve modeled 
     *by a set of points
