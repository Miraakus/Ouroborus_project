...
var Util = {
     ...
     *@return {Array} - the new rectangle increased :)
     **/
	...
	* points = [p1, p2,p3]
	* pt = [10,2,2 4,7] /*the pattern 10 dots, 2 spaces, 2 dots, 7 spaces, etc
	...
	*/
	decorate : function(ctx, points, pattern){
		...
		var pt_left = pt[0]; // spaces or dots left to paint from current index position in pattern
    ...
    lineIntersectsLine: function(l1,  l2){
        ...
        // check normal case - both lines are not vertical
            ...
            if(a1 == a2) { //parallel lines
				...
				:
                    // not coincide parallel lines have no chance to intersect
				false;
            } else { //usual case - non parallel, the 'infinite' lines intersects...we only need to know if inside the segment
                /*
    ...
    /**Tests if 3 points are colinear (similar to geometry ...with infinite lines)
     *@param {Point} p1 - first point
     *@param {Point} p2 - second point
     *@param {Point} p3 - third point
     *@return {Boolean} - true if colinear and false if not
     *@deprecated
     **/
    deprecated_collinearity: function(p1, p2, p3){
        //Check if 2 points coincide. If they do we automatically have colinearity
    ...
    /**Tests if 3 points are colinear with matrix determinants.
    ...
    isPointInside:function(point, points){
        ...
        var counter = 0;
        var p1 = points[0];
        //calculates horizontal intersects
        for (var i=1; i<=points.length; i++) {
     ...
     * @return {Number}
     *  0 - If the point lies on a part of the curve or no intersection
     ...
     * @param {Number} xc0 coordinates x for point C0 (first control point)
     * @param {Number} yc0 coordinates y for point C0 (first control point)
     * @param {Number} xc1 coordinates x for point C1 (second control point)
     * @param {Number} yc1 coordinates y for point C1 (second control point)
    ...
    /**Tries to cut the unnecessary points.
    ...
    /**Score a orthogonal path made out of Points
     ...
     *  - if the 3rd goes back we immediately return -1
     *@param {Array} v - an array of {Point}s
     ...
     * @deprecated Since Opera 19.0.1326.47 problem disappeared. So 0.8999999999999999.toString() no longer returns "0.9"
     * */
    ...
    loadPrimitive: function (o){
        ...
        if(o.oType == 'Point'){
            ...
            result = Figure.load(o); //kind a recursively
        }
        return result;
    },
