...
ConnectorManager.prototype = {    
    ...
    connectorPaint:function(context, highlightedConnectorId){
        for(var i=0; i<this.connectors.length; i++){
            ...
            //detect if we have a currently selected Connector and "highlight" its ConnectionPoints
    ...
    connectorConnect:function(connectionPoint1, connectionPoint2){
        ...
        //select the figure we just figures connected to
        var figurePoint;
        var nonFigurePoint;
     ...
     *@return {Connetor} if founded or null if none founded
     **/
     ...
     *we iterate from 0 to connectors.length we are going from back to front, similar
     *to painting
     */
     ...
     *@param {Array} eBounds - the ending bounds (of a Figure) as [left, top, right, bottom] - area we should avoid
    ...
    connector2Points: function(type,  startPoint, endPoint, sBounds, eBounds ){
        ...
        switch(type){
            ...
            case Connector.TYPE_JAGGED:
                ...
                //FILTER solutions
                /*Algorithm
                 * 0. solutions are ordered from minimum nr of points to maximum >:)
                 ...
                 * (not interested) sort by length :p
                 */
                //1. filter non orthogonal solutions
                if(true){
     ...
     *  on the same line we add +1 if the 3rd is up or down related to the 2nd we do not do anything
     *  and if the 3rd goes back we immediately return -1
        ...
        if (DIAGRAMO.visualMagnet) {
            if (currentCloud.length) { //draw only if we have a cloud
                ...
                if (context.ellipse) {  // if context.ellipse() function implemented
                    ...
                    * Algorithm described in test/issues/3/Demo.html
                    * */
