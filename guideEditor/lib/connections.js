...
Connector.prototype = {    
    ...
    paintOrganic : function(context){
        ...
        //Draw NURB based only on turning points
    ...
    /**
     *Remove redundant points (we have just adjusted one of the handles of this figure, so)
     **/
    redraw:function(){
        if(this.type=='jagged'){
            var changed=true;
     ...
     * Important to remember is that by moving and edge turning point all their might be cases when more than one
     * points need to change
     * Once a figure is changed its ConnectionPoints got transformed...so the glued Connector must
    ...
    adjust:function(matrix, point){
        ...
        if(this.type == Connector.TYPE_JAGGED){
            ...
            for(var i=start; i!=end; i+=direction){
                ...
                //don`t do this if they are our start/end point
    ...
    addUserChange: function(userChange) {
        var changesLength = this.userChanges.length;
        var currentChange;
        /*First search if we need to merge current change with existing one,
     ...
     *Useful for setting up the middle text
    ...
    middle:function(){
        if(this.type == Connector.TYPE_STRAIGHT){
            ...
            /** Algorithm:
             * Find the length of the connector. Then go on each segment until we will reach half of the
             * connector's length.
             **/
            //find total distance
            var distance = 0;
            ...
            //we have the middle distance somewhere between index and index+1
            if(index != -1){
 ...
 *@param {Number} cp2Id - the id of the second {ConnectionPoint} (usually from a {Connector})
...
function Glue(cp1Id,cp2Id,automatic){
    ...
    this.id1 = cp1Id;    
    /**Second shape's id (usually from a {Connector})*/
    this.id2 = cp2Id;
